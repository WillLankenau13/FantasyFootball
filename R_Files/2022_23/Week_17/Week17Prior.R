...
Yahoo <- read_csv(eval(paste("~/R Stuff/FantasyFootball/Yahoo/", Year, "/Week_", Char_Week, "_Yahoo.csv", sep = ""))) %>% 
  select(ID:Starting) %>% 
...
Fantasy_2021 <- Fantasy_2021 %>% 
  full_join(Yahoo, by = c("player" = "full_name"))
...
Receiving_Stats_2021 <- full_join(Receiving_Stats_2021, Passing_Defense_2021, by = c("Long_Name.y" = "Tm")) %>% 
  ...
  mutate(ovr_ex_rec = (pl_rec_game + def_ex_cmp) / 2,
         ovr_ex_rec_yds = (pl_yds_game + def_ex_yds) / 2,
         ovr_ex_rec_td = (pl_td_game + def_ex_td) / 2) %>% 
  mutate(expected_points = ovr_ex_rec*0.5 + ovr_ex_rec_yds*0.1 + ovr_ex_rec_td*6 - 2*fl/games)

Receiving_Expected_Points_Prior <- Receiving_Stats_2021 %>% 
  select(player, fant_pos, Long_Name.x, Opponent, expected_points, Salary, ovr_ex_rec, ovr_ex_rec_yds, ovr_ex_rec_td, fl) %>% 
...
RB_Stats_2021 <- full_join(RB_Stats_2021, Rushing_Defense_2021, by = c("Long_Name.y" = "Tm")) %>%
  ...
  mutate(expected_points = ovr_ex_rec*0.5 + ovr_ex_rec_yds*0.1 + ovr_ex_rus_yds*0.1 + ovr_ex_rec_td*6 + ovr_ex_rus_td*6- 2*fl/games)

RB_Expected_Points_Prior <- RB_Stats_2021 %>% 
  select(player, fant_pos, Long_Name.x, Opponent, expected_points, Salary, ovr_ex_rec_yds, ovr_ex_rec_td, ovr_ex_rus_yds, ovr_ex_rus_td, ovr_ex_rec, fl) %>% 
  mutate(ex_ppd = expected_points/Salary) %>% 
  filter(Salary > 10)
...
QB_Expected_Points_Prior <- QB_Stats_2021 %>% 
  select(player, fant_pos, Long_Name.x, Opponent, expected_points, Salary, ovr_ex_pas_yds, ovr_ex_pas_td, ovr_ex_rus_yds, ovr_ex_rus_td, fl, pl_int_game) %>% 
  mutate(ex_ppd = expected_points/Salary) %>% 
  filter(Salary > 10)
...
write_csv(QB_Expected_Points_Prior, (eval(paste("~/R Stuff/FantasyFootball/Prior/", Year, "/Week_", Char_Week, "/Week_", Char_Week, "_QB_Prior.csv", sep = ""))))
write_csv(RB_Expected_Points_Prior, (eval(paste("~/R Stuff/FantasyFootball/Prior/", Year, "/Week_", Char_Week, "/Week_", Char_Week, "_RB_Prior.csv", sep = ""))))
write_csv(Receiving_Expected_Points_Prior, (eval(paste("~/R Stuff/FantasyFootball/Prior/", Year, "/Week_", Char_Week, "/Week_", Char_Week, "_Rec_Prior.csv", sep = ""))))
