...
QB_Expected_Points <- QB_Stats %>% 
  select(Player, Position_yah, Team_yah, Opponent_yah, expected_points, Salary_yah, ovr_ex_pas_yds, ovr_ex_pas_td, ovr_ex_rus_yds, ovr_ex_rus_td, fmb_scrim, Int_pas) %>% 
...
RB_Expected_Points <- RB_Stats %>% 
  select(Player, Position_yah, Team_yah, Opponent_yah, expected_points, Salary_yah, ovr_ex_rec, ovr_ex_rec_yds, ovr_ex_rec_td, ovr_ex_rus_yds, ovr_ex_rus_td, fmb_scrim) %>% 
  mutate(ex_ppd = expected_points/Salary_yah) %>% 
  filter(Salary_yah > 10) %>% 
  mutate(home_away = ifelse(Team_yah %in% Home_Away$Home, "Home", "Away"))
...
Receiving_Expected_Points <- Receiving_Stats %>% 
  select(Player, Position_yah, Team_yah, Opponent_yah, expected_points, Salary_yah, ovr_ex_rec, ovr_ex_rec_yds, ovr_ex_rec_td, fmb_scrim) %>% 
  mutate(ex_ppd = expected_points/Salary_yah) %>% 
  filter(Salary_yah > 10) %>% 
  mutate(home_away = ifelse(Team_yah %in% Home_Away$Home, "Home", "Away"))
